...
struct ImTextureRef
{
    ...
    inline ImTextureID GetTexID() const;  // == (_TexData ? _TexData->TexID : _TexID) // Implemented
                                          // below in the file.
...
TextUnformatted(const char* text,
                const char* text_end =
                    NULL);  // raw text without formatting. Roughly equivalent to Text("%s", text) but: A) doesn't
                            // require null terminated string if 'text_end' is specified, B) it's faster, no memory copy
                            // is done, no buffer size limits, recommended for long chunks of text.
...
IMGUI_API bool TreeNode(const char* str_id, const char* fmt, ...) IM_FMTARGS(
    2);  // helper variation to easily decorrelate the id from the displayed string. Read the FAQ about why and how to
...
CollapsingHeader(const char* label,
                 ImGuiTreeNodeFlags flags = 0);  // if returning 'true' the header is open. doesn't indent nor push on
...
GetKeyPressedAmount(ImGuiKey key, float repeat_delay,
                    float rate);  // uses provided repeat rate/delay. return a count, most often 0 or 1 but might be >1
                                  // if RepeatRate is small enough that DeltaTime > RepeatRate
IMGUI_API const char*
GetKeyName(ImGuiKey key);  // [DEBUG] returns English name of the key. Those names are provided for debugging purpose
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_NoNavFocus = 1 << 17,       // No focusing toward this window with keyboard/gamepad navigation
...
enum ImGuiItemFlags_
{
    ...
    ImGuiItemFlags_AutoClosePopups = 1 << 4,    // true     // MenuItem()/Selectable() automatically close their
                                                // parent popup window.
    ImGuiItemFlags_AllowDuplicateId = 1 << 5,   // false    // Allow submitting an item with the same identifier as
                                                // an item already submitted this frame without triggering a
                                                // warning tooltip if io.ConfigDebugHighlightIdConflicts is set.
    ImGuiItemFlags_Disabled = 1 << 6,           // false    // [Internal] Disable interactions. DOES NOT affect
...
enum ImGuiInputTextFlags_
{
    ...
    // - Not well tested yet. Please report any incorrect cursor movement, selection
    // behavior etc. bug to https://github.com/ocornut/imgui/issues/3237.
    ...
    //   Ballpark estimate of cost on my 2019 desktop PC: for a 100 KB text buffer:
    //   +~0.3 ms (Optimized) / +~1.0 ms (Debug build). The CPU cost is very roughly
    //   proportional to text length, so a 10 KB buffer should cost about ten times
    //   less.
...
enum ImGuiTreeNodeFlags_
{
    ...
    ImGuiTreeNodeFlags_SpanAvailWidth = 1 << 11,    // Extend hit box to the right-most edge, even if not framed. This
                                                    // is not the default in order to allow adding other items on the
                                                    // same line without using AllowOverlap mode.
    ImGuiTreeNodeFlags_SpanFullWidth = 1 << 12,     // Extend hit box to the left-most and right-most edges (cover the
                                                    // indent area).
    ImGuiTreeNodeFlags_SpanLabelWidth = 1 << 13,    // Narrow hit box + narrow hovering highlight, will only cover the
                                                    // label text.
...
enum ImGuiPopupFlags_
{
    ...
    ImGuiPopupFlags_MouseButtonLeft = 1 << 2,    // For BeginPopupContext*(): open on Left Mouse release. Only one
                                                 // button allowed!
    ImGuiPopupFlags_MouseButtonRight = 2 << 2,   // For BeginPopupContext*(): open on Right Mouse release. Only one
                                                 // button allowed! (default)
    ImGuiPopupFlags_MouseButtonMiddle = 3 << 2,  // For BeginPopupContext*(): open on Middle Mouse release. Only one
                                                 // button allowed!
    ...
    ImGuiPopupFlags_InvalidMask_ = 0x03,      // [Internal] Reserve legacy bits 0-1 to detect incorrectly passing 1
...
enum ImGuiSelectableFlags_
{
    ...
    ImGuiSelectableFlags_SelectOnNav = 1 << 6,   // Auto-select when moved into, unless Ctrl is held. Automatic when
                                                 // in a BeginMultiSelect() block.
...
enum ImGuiTabItemFlags_
{
    ...
    ImGuiTabItemFlags_Trailing = 1 << 7,          // Enforce the tab position to the right of the tab bar (before the
                                                  // scrolling buttons)
...
enum ImGuiFocusedFlags_
{
    ...
    // ImGuiFocusedFlags_DockHierarchy               = 1 << 4,   // Consider docking
    // hierarchy (treat dockspace host as parent of docked window) (when used with
...
enum ImGuiHoveredFlags_
{
    ...
    // ImGuiHoveredFlags_AllowWhenBlockedByModal     = 1 << 6,   // Return true even
    // if a modal popup window is normally blocking access to this item/window.
    ...
    // - for frequently actioned or hovered items providing a tooltip, you want may
    // to use ImGuiHoveredFlags_ForTooltip (stationary + delay) so the tooltip
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_AcceptDrawAsHovered = 1 << 13,     // Accepting item will render as if hovered. Useful for e.g. a
...
enum ImGuiDataType_
{
    ...
    ImGuiDataType_Bool,    // bool (provided for user convenience, not supported by
                           // scalar widgets)
    ImGuiDataType_String,  // char* (provided for user convenience, not supported by
                           // scalar widgets)
    ImGuiDataType_COUNT
};
...
enum ImGuiKey : int
{
    ...
    ImGuiKey_GamepadFaceLeft,     // X           | Y       | Square   | Tap: Toggle
                                  ...
                                  // windows)
    ImGuiKey_GamepadFaceRight,    // B           | A       | Circle   | Cancel / Close
                                  // / Exit
    ImGuiKey_GamepadFaceUp,       // Y           | X       | Triangle | Text Input /
                                  // On-screen Keyboard
    ImGuiKey_GamepadFaceDown,     // A           | B       | Cross    | Activate / Open
                                  // / Toggle / Tweak
    ...
    // [Internal] If you need to iterate all keys (for e.g. an input mapper) you may
    // use ImGuiKey_NamedKey_BEGIN..ImGuiKey_NamedKey_END.
    ...
    // - Those are written back into io.KeyCtrl, io.KeyShift, io.KeyAlt, io.KeySuper
    // for convenience,
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    ImGuiKey_COUNT = ImGuiKey_NamedKey_END,  // Obsoleted in 1.91.5 because it was misleading
...
enum ImGuiInputFlags_
{
    ...
    // Flags for Shortcut(), SetNextItemShortcut()
    // - Routing policies: RouteGlobal+OverActive >> RouteActive or RouteFocused (if
    // owner is active item) >> RouteGlobal+OverFocused >> RouteFocused (if in
...
enum ImGuiConfigFlags_
{
    ...
    ImGuiConfigFlags_NoKeyboard = 1 << 6,           // Instruct dear imgui to disable keyboard inputs and interactions.
                                                    // This is done by ignoring keyboard events and clearing existing
                                                    // states.

    // User storage (to allow your backend/engine to communicate to code that may be
    // shared between multiple projects. Those flags are NOT used by core Dear
    // ImGui)
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_RendererHasVtxOffset = 1 << 3,  // Backend Renderer supports ImDrawCmd::VtxOffset. This enables
                                                      ...
                                                      // indices.
    ImGuiBackendFlags_RendererHasTextures = 1 << 4,   // Backend Renderer supports ImTextureData requests to
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_NoAlpha = 1 << 1,          //              // ColorEdit, ColorPicker, ColorButton: ignore
                                                   ...
                                                   //              the input pointer).
    ImGuiColorEditFlags_NoPicker = 1 << 2,         //              // ColorEdit: disable picker when clicking on color
                                                   //              square.
    ...
    ImGuiColorEditFlags_NoSidePreview = 1 << 8,    //              // ColorPicker: disable bigger color preview on
                                                   ...
                                                   //              preview instead.
    ImGuiColorEditFlags_NoDragDrop = 1 << 9,       //              // ColorEdit: disable drag and drop target/source.
                                                   //              ColorButton: disable drag and drop source.
    ImGuiColorEditFlags_NoBorder = 1 << 10,        //              // ColorButton: disable border (which is enforced
                                                   //              by default)
    ImGuiColorEditFlags_NoColorMarkers = 1 << 11,  //              // ColorEdit: disable rendering R/G/B/A color
    ...
    ImGuiColorEditFlags_AlphaOpaque = 1 << 12,       //              // ColorEdit, ColorPicker, ColorButton: disable
                                                     //              alpha in the preview,. Contrary to _NoAlpha it may
                                                     //              still be edited when calling
                                                     //              ColorEdit4()/ColorPicker4(). For ColorButton()
                                                     //              this does the same as _NoAlpha.
    ImGuiColorEditFlags_AlphaNoBg = 1 << 13,         //              // ColorEdit, ColorPicker, ColorButton: disable
    ...
    ImGuiColorEditFlags_AlphaBar = 1 << 18,        //              // ColorEdit, ColorPicker: show vertical alpha
    ...
    // SetColorEditOptions(). The intent is that you probably don't want to override
    // them in most of your calls. Let the user choose via the option menu and/or
...
enum ImGuiSliderFlags_
{
    ...
    ImGuiSliderFlags_ColorMarkers = 1 << 12,    // DragScalarN(), SliderScalarN(): Draw R/G/B/A color markers on
                                                // each component.
...
enum ImGuiTableFlags_
{
    ...
    ImGuiTableFlags_NoSavedSettings = 1 << 4,    // Disable persisting columns order, width, visibility and sort
    ...
    ImGuiTableFlags_NoClip = 1 << 20,        // Disable clipping rectangle for every individual columns (reduce
...
enum ImGuiTableColumnFlags_
{
    ...
    ImGuiTableColumnFlags_NoClip = 1 << 8,        // Disable clipping for this column (all NoClip columns will render
    ...
    ImGuiTableColumnFlags_PreferSortAscending = 1 << 14,  // Make the initial sort direction Ascending when first
    ...
    ImGuiTableColumnFlags_IndentEnable = 1 << 16,   // Use current Indent value when entering cell (default for column
                                                    // 0).
...
struct ImGuiStyle
{
    ...
    float FontScaleDpi;   // Additional global scale factor from viewport/monitor
                          ...
                          // io.ConfigDpiScaleFonts is enabled, this is automatically
    ...
    float WindowBorderSize;                  // Thickness of border around windows. Generally set to
    ...
    float WindowBorderHoverPadding;          // Hit-testing extent outside/inside resizing
                                             // border. Also extend determination of hovered
                                             // window. Generally meaningfully larger than
                                             // WindowBorderSize to make it easy to reach
                                             // borders.
    ...
    ImGuiDir WindowMenuButtonPosition;       // Side of the collapsing/docking button in
                                             // the title bar (None/Left/Right). Defaults
                                             // to ImGuiDir_Left.
    float ChildRounding;                     // Radius of child window corners rounding. Set to 0.0f to
    ...
    ImVec2 ItemInnerSpacing;                 // Horizontal and vertical spacing between within
                                             ...
                                             // its label).
    ImVec2 CellPadding;                      // Padding within a table cell. Cellpadding.x is locked for
                                             // entire table. CellPadding.y may be altered between
                                             // different rows.
    ...
    float ScrollbarPadding;                  // Padding of scrollbar grab within its frame (same for
                                             // both axes).
    ...
    float TabCloseButtonMinWidthSelected;    // -1: always visible. 0.0f: visible when
                                             // hovered. >0.0f: visible when hovered
                                             // if minimum width.
    float TabCloseButtonMinWidthUnselected;  // -1: always visible. 0.0f: visible when
                                             // hovered. >0.0f: visible when hovered
                                             // if minimum width. FLT_MAX: never show
                                             // close button when unselected.
    ...
    ImGuiTreeNodeFlags TreeLinesFlags;       // Default way to draw lines connecting TreeNode hierarchy.
                                             // ImGuiTreeNodeFlags_DrawLinesNone or
    ...
    float ColorMarkerSize;                   // Size of R/G/B/A color markers for ColorEdit4() and for
    ...
    bool AntiAliasedLines;                   // Enable anti-aliased lines/borders. Disable if you are
                                             // really tight on CPU/GPU. Latched at the beginning of
    ...
    float CurveTessellationTol;              // Tessellation tolerance when using
                                             // PathBezierCurveTo() without a specific number of
                                             // segments. Decrease for highly tessellated curves
                                             // (higher quality, more polygons), increase to
                                             // reduce quality.
    ...
    IMGUI_API void ScaleAllSizes(float scale_factor);  // Scale all spacing/padding/thickness
...
struct ImGuiIO
{
    ...
    // Font system
    ImFontAtlas* Fonts;         // <auto>           // Font atlas: load, rasterize and pack
                                // one or more fonts into a single texture.
    ImFont* FontDefault;        // = NULL           // Font to use on NewFrame(). Use NULL
                                // to uses Fonts->Fonts[0].
    ...
    bool ConfigNavMoveSetMousePos;         // = false          // Directional/tabbing
                                           // navigation teleports the mouse cursor. May be
                                           // useful on TV/console systems where moving a
                                           // virtual mouse is difficult. Will update
    ...
    bool ConfigNavEscapeClearFocusWindow;  // = false          // Pressing Escape can
                                           // clear focused window as well (super set
    ...
    bool ConfigMacOSXBehaviors;               // = defined(__APPLE__) // Swap Cmd<>Ctrl keys + OS
                                              // X style text editing cursor movement using Alt
                                              // instead of Ctrl, Shortcuts using Cmd/Super
    ...
    float KeyRepeatDelay;           // = 0.275f         // When holding a key/button, time
                                    // before it starts repeating, in seconds (for buttons in
                                    // Repeat mode, etc.).
    ...
    // - Programmer seats: keep asserts (default), or disable asserts and keep error
    // tooltips (new and nice!)
    ...
    // ErrorRecoveryStoreState(), disable assert, set log callback (to e.g. trigger
    // high-level breakpoint), recover with ErrorRecoveryTryToRecoverState(),
    // restore settings.
    ...
    // order to maximize its discoverability.
    ...
    //   refer to ImOsIsDebuggerPresent() imgui_test_engine/imgui_te_utils.cpp for a
    ...
    bool ConfigDebugHighlightIdConflicts;                // = true           // Highlight and show
    ...
    // - This is inconsistent with other BeginXXX functions and create confusion for
    // many users.
    ...
    bool ConfigDebugBeginReturnValueOnce;  // = false          // First-time calls to
                                           ...
                                           // NEEDS TO BE SET AT APPLICATION BOOT
    ...
    bool ConfigDebugIgnoreFocusLoss;  // = false          // Ignore
                                      ...
                                      // calling
                                      // io.ClearInputKeys()/io.ClearInputMouse() in
                                      // input processing.

    // Option to audit .ini data
    bool ConfigDebugIniSettings;  // = false          // Save .ini data with extra
                                  // comments (particularly helpful for Docking, but
                                  // makes saving slower)
    ...
    IMGUI_API void AddInputCharacterUTF16(ImWchar16 c);  // Queue a new character input from a
    ...
    bool WantCaptureKeyboard;   // Set when Dear ImGui will use keyboard inputs, in
                                ...
                                // game/application (either way, always pass keyboard
                                // inputs to imgui). (e.g. InputText active, or an
                                // imgui window is focused and navigation is enabled,
                                // etc.).
    ...
    float Framerate;            // Estimate of application framerate (rolling average over 60
                                // frames, based on io.DeltaTime), in frame per second. Solely
                                // for convenience. Slow applications may not want to use a
                                // moving average or may want to reset underlying buffers
                                // occasionally.
    ...
    ImVec2 MouseDelta;          // Mouse delta. Note that this is zero if either current or
                                ...
                                // (-FLT_MAX,-FLT_MAX), so a disappearing/reappearing mouse
                                // won't have a huge delta.
    ...
    // Main Input State
    // (this block used to be written by backend, since 1.87 it is best to NOT write
    // to those directly, call the AddXXX functions above instead) (reading from
    // those variables is fair game, as they are extremely unlikely to be moving
    // anywhere)
    ...
    ImGuiKeyChord KeyMods;                           // Key mods flags (any of
                                                     // ImGuiMod_Ctrl/ImGuiMod_Shift/ImGuiMod_Alt/ImGuiMod_Super flags,
                                                     // same as io.KeyCtrl/KeyShift/KeyAlt/KeySuper but merged into
                                                     // flags). Read-only, updated by NewFrame()
    ImGuiKeyData KeysData[ImGuiKey_NamedKey_COUNT];  // Key state for all known keys. MUST
                                                     // use 'key - ImGuiKey_NamedKey_BEGIN'
                                                     // as index. Use IsKeyXXX() functions to
                                                     // access this.
    ...
    ImU16 MouseClickedCount[5];                      // == 0 (not clicked), == 1 (same as MouseClicked[]),
                                                     // == 2 (double-clicked), == 3 (triple-clicked) etc.
    ...
    bool MouseDownOwnedUnlessPopupClose[5];          // Track if button was clicked inside a
                                                     // dear imgui window.
    bool MouseWheelRequestAxisSwap;                  // On a non-Mac system, holding Shift requests
                                                     // WheelY to perform the equivalent of a WheelX
                                                     // event. On a Mac system this is already
                                                     // enforced by the system.
    bool MouseCtrlLeftAsRightClick;                  // (OSX) Set to true when the current click was
                                                     // a Ctrl+Click that spawned a simulated right
                                                     // click
    float MouseDownDuration[5];                      // Duration the mouse button has been down (0.0f ==
                                                     // just clicked)
    float MouseDownDurationPrev[5];                  // Previous time the mouse button has been down
    float MouseDragMaxDistanceSqr[5];                // Squared maximum distance of how much mouse
                                                     // has traveled from the clicking point (used
    ...
    // >512. bool KeysDown[ImGuiKey_COUNT]; // [LEGACY] Input: Keyboard keys that
    // are pressed (ideally left in the "native" order your engine has access to
    // keyboard keys, so you can use your own defines/enums for keys). This used to
    ...
    // Legacy: before 1.91.1, clipboard functions were stored in ImGuiIO instead of
    // ImGuiPlatformIO. As this is will affect all users of custom engines/backends,
    // we are providing proper legacy redirection (will obsolete).
    const char* (*GetClipboardTextFn)(void* user_data);
    void (*SetClipboardTextFn)(void* user_data, const char* text);
    void* ClipboardUserData;

    // void ClearInputCharacters() { InputQueueCharacters.resize(0); } // [Obsoleted
    // in 1.89.8] Clear the current frame text input buffer. Now included within
    // ClearInputKeys(). Removed this as it is ambiguous/misleading and generally
...
struct ImGuiInputTextCallbackData
{
    ...
    // our own internal data (it is not the same as your buffer)! Changes to it will
    // be reflected into your own buffer shortly after the callback.
...
enum ImGuiListClipperFlags_
{
    ...
    ImGuiListClipperFlags_NoSetTableRowCounters = 1 << 0,  // [Internal] Disabled modifying table row counters. Avoid
...
struct ImGuiListClipper
{
    ...
    double StartPosY;             // [Internal] Cursor position at the time of Begin() or after
    ...
    // case the cursor won't be advanced in the final step, and you can call
    ...
    IMGUI_API void IncludeItemsByIndex(int item_begin,
                                       int item_end);  // item_end is exclusive e.g. use (42, 42+1)
...
enum ImGuiMultiSelectFlags_
{
    ...
    ImGuiMultiSelectFlags_SelectOnClickRelease = 1 << 14,  // Apply selection on mouse release when clicking an
                                                           ...
                                                           // altering selection.
    // ImGuiMultiSelectFlags_RangeSelect2d       = 1 << 15,  // Shift+Selection uses
    // 2d geometry instead of linear sequence, so possible to use Shift+up/down to
    // select vertically in grid. Analogous to what BoxSelect does.
    ImGuiMultiSelectFlags_NavWrapX = 1 << 16,  // [Temporary] Enable navigation wrapping on X axis. Provided as a
                                               // convenience because we don't have a design for the general Nav
                                               // API for this yet. When the more general feature be public we
...
struct ImGuiMultiSelectIO
{
    ...
    ImGuiSelectionUserData NavIdItem;          //  ms:w, app:r     /                // (If using deletion) Last
                                               //  known SetNextItemSelectionUserData() value for NavId (if
                                               //  part of submitted items).
    bool NavIdSelected;                        //  ms:w, app:r     /        app:r   // (If using deletion)
                                               //  Last known selection state for NavId (if part of
                                               //  submitted items).
    bool RangeSrcReset;                        //        app:w     /  ms:r          // (If using deletion)
                                               //        Set before EndMultiSelect() to reset ResetSrcItem
...
struct ImGuiSelectionRequest
{
    ...
    ImS8 RangeDirection;                    //                  /  ms:w  app:r   // Parameter for
                                            ...
                                            //                  RangeLastItem, -1 otherwise. Useful if
                                            //                  you want to preserve selection order
...
struct ImGuiSelectionBasicStorage
{
    ...
    bool PreserveOrder;  // = false  // GetNextSelectedItem() will return ordered
                         // selection (currently implemented by two additional sorts
    ...
    ImGuiStorage _Storage;                        // [Internal] Selection set. Think of this as similar to
                                                  // e.g. std::set<ImGuiID>. Prefer not accessing
                                                  // directly: iterate with GetNextSelectedItem().

    // Methods
    IMGUI_API ImGuiSelectionBasicStorage();
    IMGUI_API void ApplyRequests(ImGuiMultiSelectIO* ms_io);  // Apply selection requests coming from
...
struct ImDrawCmd
{
    ...
    // for an upcoming feature) Since 1.92: removed ImDrawCmd::TextureId field, the
    // getter function must be used!
...
enum ImDrawFlags_
{
    ...
    ImDrawFlags_RoundCornersTopLeft = 1 << 4,      // AddRect(), AddRectFilled(), PathRect(): enable rounding top-left
...
struct ImDrawList
{
    ...
    // ImDrawListSharedData so you can use ImDrawList without ImGui, but that's more
    // involved)
    ...
    // radius_y, ImU32 col, float rot = 0.0f, int num_segments = 0, float thickness
    // = 1.0f) { AddEllipse(center, ImVec2(radius_x, radius_y), col, rot,
    // num_segments, thickness); } // OBSOLETED in 1.90.5 (Mar 2024) inline  void
    // AddEllipseFilled(const ImVec2& center, float radius_x, float radius_y, ImU32
    // col, float rot = 0.0f, int num_segments = 0) { AddEllipseFilled(center,
    // ImVec2(radius_x, radius_y), col, rot, num_segments); } // OBSOLETED in 1.90.5
    // (Mar 2024) inline  void  PathEllipticalArcTo(const ImVec2& center, float
    // radius_x, float radius_y, float rot, float a_min, float a_max, int
    // num_segments = 0) { PathEllipticalArcTo(center, ImVec2(radius_x, radius_y),
    // rot, a_min, a_max, num_segments); } // OBSOLETED in 1.90.5 (Mar 2024) inline
    // void AddBezierCurve(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3,
    // const ImVec2& p4, ImU32 col, float thickness, int num_segments = 0) {
    // AddBezierCubic(p1, p2, p3, p4, col, thickness, num_segments); } // OBSOLETED
    // in 1.80 (Jan 2021) inline  void  PathBezierCurveTo(const ImVec2& p2, const
    // ImVec2& p3, const ImVec2& p4, int num_segments = 0) {
    // PathBezierCubicCurveTo(p2, p3, p4, num_segments); } // OBSOLETED in 1.80 (Jan
    // 2021)
...
struct ImDrawData
{
    ...
    ImVec2 DisplayPos;                   // Top-left position of the viewport to render (== top-left
                                         ...
                                         // GetMainViewport()->Pos for the main viewport, == (0.0) in
                                         // most single-viewport applications)
    ...
    ImVector<ImTextureData*>* Textures;  // List of textures to update. Most of the times the list is
                                         // shared by all ImDrawData, has only 1 texture and it doesn't
                                         // need any update. This almost always points to
                                         // ImGui::GetPlatformIO().Textures[]. May be overridden or set to
                                         // NULL if you want to manually update textures.

    // Functions
    ImDrawData() { Clear(); }
    IMGUI_API void Clear();
    IMGUI_API void AddDrawList(ImDrawList* draw_list);      // Helper to add an external draw list into
                                                            // an existing ImDrawData.
...
enum ImTextureFormat
{
    ...
    ImTextureFormat_Alpha8,  // 1 component per pixel, each is unsigned 8-bit. Total
...
enum ImTextureStatus
{
    ...
    ImTextureStatus_WantCreate,   // Requesting backend to create the texture. Set
                                  // status OK when done.
    ImTextureStatus_WantUpdates,  // Requesting backend to update specific blocks of
                                  // pixels (write to texture portions which have
                                  // never been used before). Set status OK when
                                  // done.
...
struct ImTextureData
{
    //------------------------------------------ core / backend
    //---------------------------------------
    int UniqueID;                     // w    -   // [DEBUG] Sequential index to facilitate identifying
                                      // a texture when debugging/printing. Unique per atlas.
    ...
    bool WantDestroyNextFrame;        // rw   -   // [Internal] Queued to set
                                      // ImTextureStatus_WantDestroy next frame. May still
...
struct ImFontConfig
{
    ...
    bool PixelSnapH;                    // false    // Align every glyph AdvanceX to pixel boundaries.
                                        // Prevents fractional font size from working correctly! Useful
                                        // e.g. if you are merging a non-pixel aligned font with the
                                        // default font. If enabled, OversampleH/V will default to 1.
    ...
    const ImWchar* GlyphRanges;         // NULL     // *LEGACY* THE ARRAY DATA NEEDS TO PERSIST AS
                                        // LONG AS THE FONT IS ALIVE. Pointer to a user-provided list
                                        // of Unicode range (2 value per range, values are inclusive,
                                        // zero-terminated list).
    const ImWchar* GlyphExcludeRanges;  // NULL     // Pointer to a small user-provided list of
                                        // Unicode ranges (2 value per range, values are
                                        // inclusive, zero-terminated list). This is very close
                                        // to GlyphRanges[] but designed to exclude ranges from
                                        // a font source, when merging fonts with overlapping
                                        // glyphs. Use "Input Glyphs Overlap Detection Tool" to
    ...
    ImVec2 GlyphOffset;            // 0, 0     // Offset (in pixels) all glyphs from this font
                                   // input. Absolute value for default size, other sizes will
                                   // scale this value.
    ...
    // expected that you increase/decrease font scale roughly to the inverse of
    // this, otherwise quality may look lowered.
    ...
    ImFont* DstFont;                 // Target font (as we merging fonts, multiple ImFontConfig may
                                     // target the same font)
    const ImFontLoader* FontLoader;  // Custom font backend for this source (default
                                     // source is the one stored in ImFontAtlas)
    void* FontLoaderData;            // Font loader opaque storage (per font config)

#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    bool PixelSnapV;  // true    // [Obsoleted in 1.91.6] Align Scaled GlyphOffset.y
                      // to pixel boundaries.
#endif
...
struct ImFontAtlas
{
    ...
    IMGUI_API ImFont* AddFontDefault(const ImFontConfig* font_cfg = NULL);  // Selects between AddFontDefaultVector()
                                                                            // and AddFontDefaultBitmap().
    ...
    IMGUI_API ImFont*
    AddFontDefaultBitmap(const ImFontConfig* font_cfg = NULL);  // Embedded classic pixel-clean font.
    ...
    IMGUI_API ImFont*
    AddFontFromMemoryTTF(void* font_data, int font_data_size, float size_pixels = 0.0f,
    ...
    // As we are transitioning toward a new font system, we expect to obsolete those
    // soon:
    ...
    IMGUI_API void ClearTexData();    // [OBSOLETE] Clear CPU-side copy of the texture data. Saves
    ...
    // Since 1.92: specifying glyph ranges is only useful/necessary if your backend
    // doesn't support ImGuiBackendFlags_RendererHasTextures!
    ...
    IMGUI_API const ImWchar* GetGlyphRangesChineseFull();  // Default + Half-Width + Japanese
                                                           // Hiragana/Katakana + full set of about 21000 CJK
                                                           // Unified Ideographs
    ...
    // own purpose.
    // - Since 1.92.0, packing is done immediately in the function call (previously
    // packing was done during the Build call)
    ...
    IMGUI_API void RemoveCustomRect(ImFontAtlasRectId id);  // Unregister a rectangle. Existing pixels will stay
    ...
    int TexGlyphPadding;               // FIXME: Should be called "TexPackPadding". Padding
                                       // between glyphs within texture in pixels. Defaults to 1.
                                       // If your rendering method doesn't rely on bilinear
    ...
    // [Internal]
    ImVector<ImTextureData*> TexList;  // Texture list (most often TexList.Size == 1). TexData is always
    ...
    bool TexPixelsUseColors;           // Tell whether our texture data is known to use colors
                                       // (rather than just alpha channel), in order to help
    ...
    ImVector<ImDrawListSharedData*> DrawListSharedDatas;     // List of users for this atlas. Typically one per
                                                             // Dear ImGui context.
    ...
    const ImFontLoader* FontLoader;  // Font loader opaque interface (default to use FreeType when
                                     ...
                                     // runtime.
    const char* FontLoaderName;      // Font loader name (for display e.g. in About box)
                                     // == FontLoader->Name
    void* FontLoaderData;            // Font backend opaque storage
    unsigned int FontLoaderFlags;    // Shared flags (for all fonts) for font loader. THIS IS
    ...
    ImGuiContext* OwnerContext;      // Context which own the atlas will be in charge of
    ...
    // // OBSOLETED in 1.92.0 typedef ImFontAtlasCustomRect     CustomRect; //
...
struct ImFontBaked
{
    ...
    ImVector<float> IndexAdvanceX;  // 12-16 // out // Sparse. Glyphs->AdvanceX in a
    ...
    unsigned int LoadNoRenderOnLayout : 1;  // 0  //     // Enable a two-steps mode where CalcTextSize() calls
                                            ...
                                            // advantageous if you know that the glyph is unlikely to actually be
                                            // rendered, otherwise it is slower because we'd do one query on the
    ...
    void* FontLoaderDatas;                  // 4-8   //     // Font loader opaque storage (per baked
                                            // font * sources): single contiguous buffer allocated
                                            // by imgui, passed to loader.
...
{
    ImFontFlags_None = 0,
    ImFontFlags_NoLoadError = 1 << 1,     // Disable throwing an error/assert when calling AddFontXXX() with
    ...
    ImFontFlags_LockBakedSizes = 1 << 3,  // [Internal] Disable loading new baked sizes, disable garbage
                                          // collecting current ones. e.g. if you want to lock a font to a
                                          // single size. Important: if you use this to preload given sizes,
                                          // consider the possibility of multiple font density used on Retina
                                          // display.
};
...
struct ImFont
{
    ...
    ImFontBaked* LastBaked;          // 4-8   // Cache last bound baked. NEVER USE DIRECTLY.
                                     // Use GetFontBaked().
    ...
    float LegacySize;                 // 4     // in  // Font size passed to AddFont(). Use for old
                                      // code calling PushFont() expecting to use that size. (use
                                      // ImGui::GetFontBaked() to get font baked at current bound
                                      // size).
    ImVector<ImFontConfig*> Sources;  // 16    // in  // List of sources. Pointers
                                      // within OwnerAtlas->Sources[]
    ImWchar EllipsisChar;             // 2-4   // out // Character used for ellipsis rendering
                                      // ('...'). If you ever want to temporarily swap this for
                                      // an alternative/dummy char, make sure to clear
                                      // EllipsisAutoBake.
    ImWchar FallbackChar;             // 2-4   // out // Character used if a glyph isn't found
                                      // (U+FFFD, '?')
    ImU8 Used8kPagesMap[(IM_UNICODE_CODEPOINT_MAX + 1) / 8192 / 8];  // 1 bytes if ImWchar=ImWchar16, 17 bytes if
...
inline ImTextureID ImDrawCmd::GetTexID() const
{
    ...
    if (TexRef._TexData != NULL)
        IM_ASSERT(tex_id != ImTextureID_Invalid && "ImDrawCmd is referring to "
                                                   "ImTextureData that wasn't "
                                                   "uploaded to graphics "
                                                   "system. Backend must call "
...
struct ImGuiViewport
{
    ...
    ImVec2 WorkSize;           // Work Area: Size of the viewport minus task bars, menu bars,
                               // status bars (<= Size)
...
struct ImGuiPlatformIO
{
    ...
    const char* (*Platform_GetClipboardTextFn)(ImGuiContext* ctx);  // Should return NULL on failure (e.g. clipboard
                                                                    // data is not text).
    ...
    // Optional: Notify OS Input Method Editor of the screen position of your cursor
    // for text input position (e.g. when using Japanese/Chinese IME on Windows)
    // (default to use native imm32 api on Windows)
    void (*Platform_SetImeDataFn)(ImGuiContext* ctx, ImGuiViewport* viewport, ImGuiPlatformImeData* data);
    void* Platform_ImeUserData;
    // void      (*SetPlatformImeDataFn)(ImGuiViewport* viewport,
    // ImGuiPlatformImeData* data); // [Renamed to platform_io.PlatformSetImeDataFn
    // in 1.91.1]
    ...
    // array is available here mostly because backends will want to destroy textures
    // on shutdown.
    ImVector<ImTextureData*> Textures;  // List of textures used by Dear ImGui (most often 1) + contents
...
struct ImGuiPlatformImeData
{
    ...
    bool WantTextInput;     // A widget wants text input, not necessarily IME to be
                            // visible. This is automatically set to the upcoming value
                            // of io.WantTextInput.
