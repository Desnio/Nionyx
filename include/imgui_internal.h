...
enum ImDrawTextFlags_
{
    ...
    ImDrawTextFlags_CpuFineClip = 1 << 0,  // Must be == 1/true for legacy with 'bool
...
template <typename T> struct ImPool
{
    ...
    // pool.TryGetMapData(n)) { ... } Can be avoided if you know .Remove() has never
    // been called on the pool, or AliveCount == GetMapSize()
...
{
    ImVector<ImDrawList*>* Layers[2];  // Pointers to global layers for: regular,
...
{
    // Controlled by user
    ImGuiItemFlags_ReadOnly = 1 << 11,                // false     // [ALPHA] Allow hovering interactions but underlying
    ...
    ImGuiItemFlags_NoFocus = 1 << 17,                 // false     // [EXPERIMENTAL: Not very well specced] Clicking
...
enum ImGuiButtonFlagsPrivate_
{
    ...
    ImGuiButtonFlags_PressedOnClickReleaseAnywhere = 1 << 6,  // return true on click + release even if the release
    ...
    ImGuiButtonFlags_NoTestKeyOwner = 1 << 21,     // don't test key/input owner when polling the key (note: mouse
                                                   // buttons are keys! often, the key in question will be
                                                   // ImGuiKey_MouseLeft!)
    ImGuiButtonFlags_NoFocus = 1 << 22,            // [EXPERIMENTAL: Not very well specced].
...
enum ImGuiTreeNodeFlagsPrivate_
{
    ...
    ImGuiTreeNodeFlags_UpsideDownArrow = 1 << 29,  // FIXME-WIP: Turn Down arrow into an Up arrow, for reversed trees
                                                   // (#6517)
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    ImVector<char> CallbackTextBackup;  // temporary storage for callback to support
...
enum ImGuiWindowBgClickFlags_
{
    ...
    ImGuiWindowBgClickFlags_Move = 1 << 0,  // Click on bg/void + drag to move window. Cleared by default when
...
struct ImGuiInputEvent
{
    ...
    union
    {
        ImGuiInputEventMousePos MousePos;        // if Type ==
                                                 // ImGuiInputEventType_MousePos
        ImGuiInputEventMouseWheel MouseWheel;    // if Type ==
                                                 // ImGuiInputEventType_MouseWheel
        ImGuiInputEventMouseButton MouseButton;  // if Type ==
                                                 // ImGuiInputEventType_MouseButton
        ImGuiInputEventKey Key;                  // if Type ==
                                                 // ImGuiInputEventType_Key
        ImGuiInputEventText Text;                // if Type ==
                                                 // ImGuiInputEventType_Text
        ImGuiInputEventAppFocused AppFocused;    // if Type ==
                                                 // ImGuiInputEventType_Focus
    };
...
enum ImGuiActivateFlags_
{
    ...
    ImGuiActivateFlags_FromTabbing = 1 << 3,         // Activation requested by a tabbing
                                                     // request (ImGuiNavMoveFlags_IsTabbing)
    ImGuiActivateFlags_FromShortcut = 1 << 4,        // Activation requested by an item shortcut via
                                                     // SetNextItemShortcut() function.
    ImGuiActivateFlags_FromFocusApi = 1 << 5,        // Activation requested by an api
                                                     // request (ImGuiNavMoveFlags_FocusApi)
};
...
struct IMGUI_API ImGuiTypingSelectState
{
    ...
    char SearchBuffer[64];             // Search buffer: no need to make dynamic as this search
                                       // is very transient.
    ImGuiID FocusScope;
...
{
    float OffsetNorm;  // Column start offset, normalized 0.0 (far left) -> 1.0 (far
                       // right)
...
struct IMGUI_API ImGuiMultiSelectTempData
{
    ...
    bool IsKeyboardSetRange;  // Set by BeginMultiSelect() when using
                              // Shift+Navigation. Because scrolling may be affected
                              // we can't afford a frame of lag with
                              // Shift+Navigation.
    bool NavIdPassedBy;
...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    // Calculate work rect pos/size given a set of offset (we have 1 pair of offset
    // for rect locked from last frame data, and 1 pair for currently building rect)
...
struct ImGuiContext
{
    ...
    float FontBakedScale;                // == FontBaked->Size / FontSize. Scale factor over baked
    ...
    float WindowsBorderHoverPadding;              // Padding around resizable windows for which
                                                  ...
                                                  // style.WindowBorderHoverPadding). This isn't
                                                  // so multi-dpi friendly.
    ...
    ImGuiWindow* HoveredWindowBeforeClear;        // Window the mouse is hovering. Filled
                                                  // even with _NoMouse. This is currently
    ...
    bool ActiveIdAllowOverlap;          // Active widget allows another widget to steal
                                        // active id (generally for overlapping widgets, but
                                        // not always)
    ...
    double LastKeyboardKeyPressTime;            // Record the last time a keyboard key (ignore
                                                // mouse/gamepad ones) was pressed.
    ...
    ImU32 ActiveIdUsingNavDirMask;              // Active widget will want to read those nav
                                                // move requests (e.g. can activate a button and
                                                // move away from it)
    ...
    // (IMGUI_VERSION_NUM >= 18804) : 'g.ActiveIdUsingNavInputMask |= (1 <<
    // ImGuiNavInput_Cancel);' becomes --> 'SetKeyOwner(ImGuiKey_Escape, g.ActiveId)
    ...
    ImVector<ImGuiPopupData> BeginPopupStack;        // Which level of BeginPopup() we are
    ...
    // Old name for g.NavHighlightItemUnderNav before 1.91.1 (2024/10/18) this was
    // called When user starts using keyboard/gamepad, we hide mouse hovering
    ...
    ImGuiSelectionUserData NavLastValidSelectionUserData;  // Last valid data passed to
                                                           ...
                                                           // an item that doesn't have selection data.
    ...
    ImRect NavScoringRect;                       // Rectangle used for scoring, in screen space. Based of
                                                 // window->NavRectRel[], modified for directional
                                                 // navigation scoring.
    ...
    bool ConfigNavEnableTabbing;              // = true. Enable tabbing (Tab, Shift+Tab). PLEASE
    ...
    ImGuiID DragDropAcceptIdCurr;                    // Target item id (set at the time of accepting
                                                     // the payload)
    ImGuiID DragDropAcceptIdPrev;                    // Target item id from previous frame (we need to
                                                     // store this to allow for overlapping drag and
                                                     // drop targets)
    ...
    int MultiSelectTempDataStacked;  // Temporary multi-select data size (because we
                                     // leave previous instances undestructed, we
                                     // generally don't use
    ...
    ImGuiID HoverItemUnlockedStationaryId;    // Mouse has once been stationary on
                                              // this item. Only reset after departing
                                              // the item.
    ...
    ImGuiID TempInputId;                     // Temporary text input when using Ctrl+Click on a slider,
                                             // etc.
    ...
    float SliderCurrentAccum;      // Accumulated slider delta when using navigation
                                   // controls.
    bool SliderCurrentAccumDirty;  // Has the accumulated slider delta changed since
                                   // last time we tried to apply it?
    ...
    ImGuiTextBuffer LogBuffer;  // Accumulation buffer when log to clipboard. This is pointer so
    ...
    // Debug Tools
    // (some of the highly frequently used data are interleaved in other structures
    // above: DebugBreakXXX fields, DebugHookIdInfo, DebugLocateId etc.)
    ...
    ImGuiID DebugItemPickerBreakId;  // Will call IM_DEBUG_BREAK() when encountering
                                     // this ID
    ...
    int WantTextInputNextFrame;        // Copied in EndFrame() from
                                       // g.PlatformImeData.WantTextInput. Needs to be set
                                       // for some backends (SDL3) to emit character
                                       // inputs.
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    float CurrLineTextBaseOffset;  // Baseline offset (0.0f by default on a new
                                   // line, generally == style.FramePadding.y when a
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImGuiViewportP* Viewport;    // Always set in Begin(). Inactive windows may have a
    ...
    float WindowRounding;        // Window rounding at the time of Begin(). May be clamped
                                 // lower to avoid rendering artifacts with title bar,
                                 // menu bar etc.
    float WindowBorderSize;      // Window border size at the time of Begin().
    float TitleBarHeight,
        MenuBarHeight;  // Note that those used to be function before 2024/05/28. If
                        // you have old code calling TitleBarHeight() you can change
                        // it to TitleBarHeight.
    float DecoOuterSizeX1,
        DecoOuterSizeY1;  // Left/Up offsets. Sum of non-scrolling outer decorations
    ...
    float DecoInnerSizeX1,
        DecoInnerSizeY1;  // Applied AFTER/OVER InnerRect. Specialized for Tables as
                          // they use specialized form of clipping and frozen
    ...
    signed char ResizeBorderHeld;     // Current border being held for resize (-1:
                                      // none, otherwise 0-3)
    short BeginCount;                 // Number of Begin() during the current frame (generally 0 or
    ...
    short FocusOrder;                 // Order within WindowsFocusOrder[], altered when windows are
                                      // focused.
    ...
    ImGuiWindowBgClickFlags BgClickFlags : 8;    // Configure behavior of click+dragging on window bg/void or over
                                                 // items. Default sets by io.ConfigWindowsMoveFromTitleBarOnly. If you
                                                 // use this please report in #3379.
    ...
    ImVec2 SetWindowPosVal;                      // store window position when using a non-zero Pivot
                                                 // (position set needs to be processed when we know the
                                                 // window size)
    ...
    float FontRefSize;   // This is a copy of window->CalcFontSize() at the time of
                         // Begin(), trying to phase out CalcFontSize() especially as
  ...
  public:
    ...
    // g.FontSize at the time of the last Begin() call for this window. float
    // CalcFontSize() const    { ImGuiContext& g = *Ctx; return g.FontSizeBase *
...
struct ImGuiTableColumn
{
    ...
    float WidthRequest;              // Master width absolute value when !(Flags &
                                     // _WidthStretch). When Stretch this is derived every frame
                                     // from StretchWeight in TableUpdateLayout()
    float WidthAuto;                 // Automatic width
    float WidthMax;                  // Maximum width (FIXME: overwritten by each instance)
    float StretchWeight;             // Master width weight when (Flags & _WidthStretch). Often
                                     // around ~1.0f initially.
...
struct IMGUI_API ImGuiTable
{
    ...
    float CellSpacingX1;  // Spacing between non-bordered cells. Locked in
                          // BeginTable()/Layout.
    float CellSpacingX2;
    float InnerWidth;                // User value passed to BeginTable(), see comments at the top
                                     // of BeginTable() for details.
    ...
    ImRect Bg2ClipRectForDrawCmd;      // Actual ImDrawCmd clip rect for BG2 channel.
                                       // This tends to be a correct, tight-fit, because
                                       // output to BG2 are done by widgets relying on
                                       // regular ClipRect.
    ...
    ImGuiWindow* InnerWindow;          // Window holding the table data (== OuterWindow or a
                                       // child window)
    ImGuiTextBuffer ColumnsNames;      // Contiguous buffer holding columns names
    ImDrawListSplitter* DrawSplitter;  // Shortcut to TempData->DrawSplitter while in table. Isolate
                                       // draw commands per columns to avoid switching clip rect
                                       // constantly
    ...
    ImGuiTableColumnIdx ResizedColumn;               // Index of column being resized. Reset when
                                                     // InstanceCurrent==0.
    ...
    ImGuiTableDrawChannelIdx Bg2DrawChannelCurrent;  // For Selectable() and other widgets drawing across
    ...
    bool DisableDefaultContextMenu;  // Disable default context menu. You may submit
                                     // your own using
...
struct IMGUI_API ImGuiTableTempData
{
    ...
    ImVec2 HostBackupPrevLineSize;     // Backup of InnerWindow->DC.PrevLineSize at the
                                       // end of BeginTable()
    ImVec2 HostBackupCurrLineSize;     // Backup of InnerWindow->DC.CurrLineSize at the
                                       // end of BeginTable()
    ImVec2 HostBackupCursorMaxPos;     // Backup of InnerWindow->DC.CursorMaxPos at the
                                       // end of BeginTable()
...
struct ImFontLoader
{
    ...
    // Size of backend data, Per Baked * Per Source. Buffers are managed by core to
    // avoid excessive allocations.
    // FIXME: At this point the two other types of buffers may be managed by core to
    // be consistent?
...
{
    int TargetIndex : 20;          // When Used: ImFontAtlasRectId -> into Rects[]. When unused: index
                                   // to next unused RectsIndex[] slot to consume free-list.
...
{
    stbrp_context_opaque PackContext;  // Actually 'stbrp_context' but we don't want
                                       // to define this in the header file.
    ...
    ImFontAtlasRectId PackIdMouseCursors;  // White pixel + mouse cursors. Also happen to be
